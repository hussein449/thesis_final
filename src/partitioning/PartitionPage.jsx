...
function SummaryStrip({ allocations, droneCount }) {
  ...
  const kpis = [
    ...
    { label: '∑ Risk score',  value: totalScore.toFixed(2),              hint: 'sum across all roads', dot: '#fbbf24' },
    { label: 'Top corridor',  value: `×${top.drones}`,                   hint: top.road.shortName,     dot: top.road.color },
    { label: 'Top-3 share',   value: `${concentrationTop3.toFixed(0)}%`, hint: 'fleet on busiest 3',   dot: '#60a5fa' },
    { label: 'Unpatrolled',   value: unpatrolled,                        hint: unpatrolled === 0 ? 'full coverage' : 'gap exists', dot: unpatrolled === 0 ? '#34d399' : '#f87171' },
  ]
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-xl ring-1 ring-slate-800/80 bg-slate-900/40 px-3.5 py-2.5">
      ...
      {/* Formula card */}
      <div className="rounded-xl ring-1 ring-slate-800/70 bg-slate-950/40 px-4 py-3">
        ...
        <div className="font-mono text-[11px] text-slate-200 leading-relaxed mt-1">
          R<sub>i</sub> = 0.40·A + 0.25·T + 0.20·S + 0.15·C
        </div>
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="text-[9.5px] text-[var(--color-txt3)] leading-relaxed">
          Full decomposition of R = 0.40·A + 0.25·T + 0.20·S + 0.15·C for each corridor. Normalised to [0,1].
        </div>
      </div>
      <div className="rounded-xl ring-1 ring-slate-800/80 bg-slate-900/40 px-5 py-4">
          ...
          <table className="w-full text-[10.5px] font-mono">
            ...
            <tbody>
              {allocations.map(({ road, score, drones }) => {
                ...
                return (
                  <tr key={road.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    ...
                    <td className="text-right py-2 px-2 text-red-300">{tA.contrib.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-slate-400">{tT.norm.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-amber-300">{tT.contrib.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-slate-400">{tS.norm.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-blue-300">{tS.contrib.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-slate-400">{tC.norm.toFixed(3)}</td>
                    <td className="text-right py-2 px-2 text-emerald-300">{tC.contrib.toFixed(3)}</td>
        ...
        <div className="mt-3 grid grid-cols-2 lg:grid-cols-4 gap-2 text-[9px]">
          {[
            { label: 'A — Accident history', color: 'text-red-300', note: 'accidents / 20 (ref)' },
            { label: 'T — Traffic intensity', color: 'text-amber-300', note: 'AADT / 50 000 (ref)' },
            { label: 'S — Speed contribution', color: 'text-blue-300', note: 'speed / 120 km/h (ref)' },
            { label: 'C — Pavement condition', color: 'text-emerald-300', note: '(5 − cond) / 4 · inverted' },
          ].map(({ label, color, note }) => (
            <div key={label} className="rounded-lg ring-1 ring-slate-800 bg-slate-950/40 px-2.5 py-2">
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="text-[9.5px] text-[var(--color-txt3)] leading-relaxed">
          ...
          <span className="text-emerald-400 font-bold ml-1">+</span> = largest-remainder rounding.
        </div>
      </div>
      <div className="rounded-xl ring-1 ring-slate-800/80 bg-slate-900/40 px-5 py-4">
        <div className="overflow-x-auto">
          <table className="w-full text-[10.5px] font-mono">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                {FLEET_SIZES.map(n => (
                  <th key={n} className="text-center py-2.5 px-3 text-[9px] font-semibold uppercase tracking-wider text-blue-300">
                    N = {n}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {roads.map(({ road, score, percentage }) => (
                <tr key={road.id} className="border-b border-slate-800/60 hover:bg-slate-800/30 transition-colors">
                      ...
                      <td key={n} className="text-center py-2.5 px-3">
                        <span className="inline-flex flex-col items-center gap-0.5">
                          <span className="text-[13px] font-bold tabular-nums" style={{ color: road.color }}>
                            {item.drones}
                            {isRemainder && <span className="text-emerald-400 text-[9px] ml-0.5">+</span>}
                          </span>
    ...
    <div className="space-y-3">
      <div>
        ...
        <div className="text-[9.5px] text-[var(--color-txt3)] leading-relaxed">
          ...
          <span className="inline-flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 inline-block" /><span className="text-emerald-300/90">REAL</span></span>{' '}
          <span className="inline-flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-blue-400 inline-block" /><span className="text-blue-300/90">DERIVED</span></span>{' '}
          <span className="inline-flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-amber-400 inline-block" /><span className="text-amber-300/90">ESTIMATE</span></span>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[10.5px] text-slate-300 leading-relaxed">
        <div className="rounded-xl ring-1 ring-emerald-500/20 bg-emerald-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-emerald-300/90">Speed limits — real</span>
          </div>
          Lebanese Traffic Law no. 243 (22 Oct 2012), Article 84. Cross-checked against OpenStreetMap <code className="text-emerald-300/80">maxspeed</code> tags.
        </div>
        <div className="rounded-xl ring-1 ring-emerald-500/20 bg-emerald-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-emerald-300/90">Road geometry — real</span>
          </div>
        ...
        <div className="rounded-xl ring-1 ring-blue-500/20 bg-blue-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-blue-300/90">AADT — real (range)</span>
          </div>
        ...
        <div className="rounded-xl ring-1 ring-blue-500/20 bg-blue-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-blue-300/90">Annual accidents — derived SPF</span>
          </div>
          Exposure-based SPF: E[crashes] = k·L·AADT. ISF national ~3,500–4,500 RTAs/yr. WHO 2018 Lebanon profile.
          <span className="block mt-1 text-amber-300/80 text-[9px]">Per-road exact counts are not measurements.</span>
        </div>
        <div className="rounded-xl ring-1 ring-amber-500/25 bg-amber-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400" />
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-amber-300/90">Pavement condition — estimate</span>
          </div>
        ...
        <div className="rounded-xl ring-1 ring-emerald-500/20 bg-emerald-500/[0.04] px-3 py-2.5">
          <div className="flex items-center gap-1.5 mb-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-emerald-300/90">Allocation algorithm — real</span>
          </div>
...
const NAV_ITEMS = [
  ...
  { key: 'riskscores', label: 'Risk Score Breakdown',  icon: '≡', group: 'Analysis' },
  { key: 'multifleet', label: 'Multi-fleet Table',     icon: '⊞', group: 'Analysis' },
  { key: 'datasources',label: 'Data Sources',          icon: '⊕', group: 'Reference' },
]
...
export default function PartitionPage({ droneCount, onDroneCountChange }) {
  ...
  return (
    <div className="flex flex-1 min-h-0 overflow-hidden bg-slate-950">
      ...
      {/* ── Left sidebar ── */}
      <aside className={`${sidebarOpen ? 'w-52' : 'w-10'} shrink-0 flex flex-col border-r border-slate-800/70 bg-[#070d1a] transition-all duration-200 overflow-hidden`}>

        {/* Sidebar header */}
        <div className="px-3 pt-3 pb-3 border-b border-slate-800/70 shrink-0 flex items-start justify-between gap-2">
          {sidebarOpen && (
            <div className="min-w-0">
              <div className="text-[9px] text-purple-300/70 uppercase tracking-widest font-semibold mb-0.5 truncate">
                Risk partitioning
              </div>
        ...
        {/* Fleet size control */}
        <div className={`px-4 py-3 border-b border-slate-800/70 shrink-0 ${sidebarOpen ? '' : 'hidden'}`}>
        ...
        <div className={`px-4 py-3 border-t border-slate-800/70 shrink-0 ${sidebarOpen ? '' : 'hidden'}`}>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {activeSection === 'map' && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-5">
              <div className="rounded-2xl ring-1 ring-slate-800/80 bg-slate-900/40 overflow-hidden flex flex-col">
                <div className="flex items-center gap-2.5 px-5 py-3 border-b border-slate-800/70">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />
                  ...
                  <button
                    ...
                    className={`ml-auto text-[9.5px] font-semibold px-2.5 py-1 rounded-lg ring-1 cursor-pointer transition-colors ${
                      heatmap
                        ? 'bg-orange-500/15 ring-orange-500/40 text-orange-300 hover:bg-orange-500/25'
                        : 'bg-slate-800/60 ring-slate-700 text-slate-400 hover:text-slate-200'
                    }`}
                  >
        ...
        {activeSection === 'riskscores' && (
          <>
            <SectionHeader
              icon="≡"
              title="Risk Score Breakdown"
              description="Per-corridor decomposition showing each normalised factor and its weighted contribution to the composite risk score R."
            />
        ...
        {activeSection === 'datasources' && (
          <>
            <SectionHeader
              ...
              description="Review before citing in the thesis. Each numeric input is tagged as REAL, DERIVED, or ESTIMATE."
            />
            <DataSources />
          </>
        )}
